...
\path{evidence/canonical/medium_contr/config_resolved.yaml} and
\path{evidence/canonical/medium_no_contr/config_resolved.yaml}, and analogously for gpt2-large.
For the cross-holdout replication block, we use the same model/operator settings but switch LOTO holdout to \texttt{orthogonal\_rotation} with paired seeds 1234/2028/2029.
...
The bundled runs instantiate the operator parameters as follows (from the corresponding \texttt{config\_resolved.yaml} files in the \texttt{evidence/} tree):
...
Sentiment:
\end{quote}
Accuracy is computed over 250 examples (see the corresponding \texttt{config\_resolved.yaml} files in the \texttt{empty\_dirs\_for\_codex outputs runs\_recovery} tree).
Accuracy is computed over 250 examples (see the corresponding \texttt{config\_resolved.yaml} files in the \texttt{evidence/} tree).
...
\begin{table}[h]
  \centering
  \small
  \caption{Per-task metrics for gpt2-medium under the OOD held-out \texttt{contiguous\_overwrite} protocol. Clean metrics are at $\varepsilon=0$ and corrupt metrics at $\varepsilon=0.16$. ``Best heuristic'' is smoothing in this evaluation protocol.}
  \label{tab:per_task}
  \vspace{0pt}
...
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=2pt]
  \item \path{evidence/canonical/medium_contr/metrics/task_metrics.json}
  \item \path{evidence/canonical/medium_contr/metrics/stability_metrics.json}
  \item \path{evidence/canonical/medium_no_contr/metrics/task_metrics.json}
  \item \path{evidence/canonical/medium_no_contr/metrics/stability_metrics.json}
  \item \path{evidence/second_model/gpt2_large_contr/metrics/task_metrics.json}
  \item \path{evidence/lambda/lambda5_tuned_single/metrics/task_metrics.json}
  \item \path{evidence/lambda/lambda5_tuned_single/metrics/stability_metrics.json}
  \item \path{evidence/seedrep_overwrite/*/metrics/*.json}
  \item \path{evidence/seedrep_orth/*/metrics/*.json}
\end{itemize}
\endgroup
...
\footnotesize
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=2pt]
  \item \path{evidence/canonical/medium_contr/config_resolved.yaml}
  \item \path{evidence/canonical/medium_no_contr/config_resolved.yaml}
  \item \path{evidence/second_model/gpt2_large_contr/config_resolved.yaml}
  \item \path{evidence/lambda/lambda5_tuned_single/config_resolved.yaml}
  \item \path{evidence/seedrep_overwrite/*/config_resolved.yaml}
  \item \path{evidence/seedrep_orth/*/config_resolved.yaml}
\end{itemize}
\endgroup
