...
We also study an explicit \textbf{contraction regularizer} that encourages repaired cache states to contract toward the clean-cache manifold.
...
In the canonical paired ablation that toggles only the contraction weight, contraction improves robustness AUC from 0.0388 to 0.0401 and reduces logit sensitivity relative to the no-contraction variant (Table~\ref{tab:ablations}, Fig.~\ref{fig:sens}).
However, a five-seed replication shows near-neutral average effects and bootstrap intervals that include no effect (Tables~\ref{tab:seedrep} and~\ref{tab:seedrep_ci}), and a second three-seed replication on held-out \texttt{orthogonal\_rotation} shows the same non-robust contraction pattern (Table~\ref{tab:orth3}).
A second-model run on gpt2-large shows qualitative transfer in the same direction (Table~\ref{tab:second_model}).
We therefore emphasize OOD robustness/clean tradeoffs and second-model transfer as our strongest evidence; \textbf{absolute stability versus the unmodified baseline is not the central win} in this setting.

\paragraph{Contributions.}
\begin{itemize}
  \item \textbf{Distillation-based KV repair.} We introduce CacheMedic++, a learned KV-cache stability operator $R_\phi$ inserted inside attention; base model weights remain frozen and only $R_\phi$ is trained via self-distillation.
  \item \textbf{Controlled contraction study.} We evaluate an explicit contraction regularizer with paired no\_contr comparisons and five-seed replication, finding a neutral average-case effect in this regime.
  \item \textbf{Stability as a first-class metric.} We provide reproducible protocols for logit sensitivity curves and layer/head amplification maps, and we report paired ablations and second-model transfer.
\end{itemize}
