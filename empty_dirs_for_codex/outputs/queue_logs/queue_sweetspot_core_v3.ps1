...
& $py -m cachemedicpp.eval --config configs/mve.yaml --run_dir $runMve *>&1 | Tee-Object -FilePath $mveLog
$code = $LASTEXITCODE
if ($code -eq 0) {
  & $py -m cachemedicpp.stability --config configs/mve.yaml --run_dir $runMve *>&1 | Tee-Object -FilePath $mveLog -Append
  $code = $LASTEXITCODE
}
if ($code -eq 0) {
  & $py -m cachemedicpp.plots --run_dir $runMve *>&1 | Tee-Object -FilePath $mveLog -Append
  $code = $LASTEXITCODE
}
if ($code -eq 0) {
  & $py -m cachemedicpp.sweep --sweep_config configs/fast_paper_sweep_core.yaml --out_root $outRoot *>&1 | Tee-Object -FilePath $coreLog
  $code = $LASTEXITCODE
}
if ($code -eq 0) {
  & $py -m cachemedicpp.train --config configs/fast_second_model_gpt2_large.yaml --run_dir $secondRunDir *>&1 | Tee-Object -FilePath $secondLog
  $code = $LASTEXITCODE
}
if ($code -eq 0) {
  & $py -m cachemedicpp.eval --config configs/fast_second_model_gpt2_large.yaml --run_dir $secondRunDir *>&1 | Tee-Object -FilePath $secondLog -Append
  $code = $LASTEXITCODE
}
if ($code -eq 0) {
  & $py -m cachemedicpp.stability --config configs/fast_second_model_gpt2_large.yaml --run_dir $secondRunDir *>&1 | Tee-Object -FilePath $secondLog -Append
  $code = $LASTEXITCODE
}
if ($code -eq 0) {
  & $py -m cachemedicpp.plots --run_dir $secondRunDir *>&1 | Tee-Object -FilePath $secondLog -Append
  $code = $LASTEXITCODE
}
